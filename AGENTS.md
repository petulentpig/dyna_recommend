...
4. Read `review.md`, `review.json`, and `email.txt` from the same run. Distinguish included, held, excluded, and unmapped items. Always report on the latest released version per configured channel, discovered afresh at run time. Preserve the release names/versions and rollout dates at the top of the email, the release-specific subject, and the technology ranking table. Explain that entity prevalence measures deployment presence, not traffic or business impact.
